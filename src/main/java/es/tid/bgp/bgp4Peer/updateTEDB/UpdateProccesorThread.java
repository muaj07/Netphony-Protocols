...
import es.tid.bgp.bgp4Peer.peer.CMSPostMsgs;
...
public class UpdateProccesorThread extends Thread {
	...
	public void run(){	
		...
		while (running) {
			try {
				...
				log.debug("Update Msg Received from "+learntFrom +"  Queue size: " +updateList.size());
	...
	private void processAttributeAsPath(AS_Path_Attribute asPathAtt, String learntFrom) {
		//List <AS_Path_Segment> asPathSeg= asPathAtt.getAsPathSegments();
		AS_Path_Segment asPathSeg= (asPathAtt.getAsPathSegments()).get(0);
//		ASnumber= (asPathSeg.getSegments())[0];
//
//		if (ASnumber!=0)
//		log.info("TLV ASNumber: " +ASnumber);
...
/*		try {
	...
	*/
		Inet4Address asNumber= itNodeNLRI.getLocalNodeDescriptors().getAutonomousSystemSubTLV().getAS_ID();
		ASnumber= ByteBuffer.wrap(asNumber.getAddress()).getInt();
		log.debug("In ITNode As Number: " +ASnumber);
		...
		if(simpleTEDB.getItResources()==null)
		 itResources= new IT_Resources();
			...
			//if advertiseCatalogue is true
				//send the post to Javier
				//set to adviertised
				//print domain id and entrypoint
				//http://nameOfTheContainer/mdc/domain
			...
			SetdomainPost(simpleTEDB.getItResources().getControllerIT(),ASnumber,learntFrom);
	...
	private void fillNodeInformation(NodeNLRI nodeNLRI, String learntFrom) throws UnknownHostException {
		...
		log.debug(".........................Fill Node Information.........................");
		...
		if(node_info.getLearntFrom()==null || node_info.getLearntFrom().equals(learntFrom)) {
			...
			log.debug("Value for Setting Learnt from: " + node_info.getLearntFrom());
	...
	private void SetdomainPost(String controllerIT, int nodeId, String Learntfrom) {

			new CMS_Domain_Msgs (CMS_Msg, controllerIT, nodeId, Learntfrom);
	}
