...
public class SendTopology implements Runnable {
	...
	public void configure(Hashtable<String, TEDB> intraTEDBs, BGP4SessionsInformation bgp4SessionsInformation, boolean sendTopology, int instanceId, boolean sendIntraDomainLinks, MultiDomainTEDB multiTED){
		configure(intraTEDBs, bgp4SessionsInformation, sendTopology, instanceId,sendIntraDomainLinks,multiTED, false, 0,"0.0.0.0" ,0);
	...
	public void configure( Hashtable<String,TEDB> intraTEDBs,BGP4SessionsInformation bgp4SessionsInformation,boolean sendTopology,int instanceId,boolean sendIntraDomainLinks, MultiDomainTEDB multiTED, boolean test){
		configure(intraTEDBs, bgp4SessionsInformation, sendTopology, instanceId,sendIntraDomainLinks,multiTED, test, 0, "0.0.0.0", 0);

	}



	public void configure( Hashtable<String,TEDB> intraTEDBs,BGP4SessionsInformation bgp4SessionsInformation,boolean sendTopology,int instanceId,boolean sendIntraDomainLinks, MultiDomainTEDB multiTED, int AS,String bgpIdentifier ,int pref){
		configure(intraTEDBs, bgp4SessionsInformation, sendTopology, instanceId,sendIntraDomainLinks,multiTED, false,AS,bgpIdentifier,pref);

	}


	public void configure( Hashtable<String,TEDB> intraTEDBs,BGP4SessionsInformation bgp4SessionsInformation,boolean sendTopology,int instanceId,boolean sendIntraDomainLinks, MultiDomainTEDB multiTED, boolean test, int AS,String bgpIdentifier ,int localPref){
		...
		try {
			...
			this.localBGPLSIdentifer=(Inet4Address)Inet4Address.getByName(bgpIdentifier);
...
//		}
	...
	private  BGP4Update createMsgUpdateITNodeNLRI(String domainID, IT_Resources itResources){
		try{
			...
			LocalNodeDescriptorsTLV localNodeDescriptors = new LocalNodeDescriptorsTLV();
				...
				//---Autonomous system Number used for the Messages to MultiDomaian Catalogue System
			AutonomousSystemNodeDescriptorSubTLV autonomousSystem = new AutonomousSystemNodeDescriptorSubTLV();
			autonomousSystem.setAS_ID(IntToInet4Address(this.ASnumber));
			localNodeDescriptors.setAutonomousSystemSubTLV(autonomousSystem);
	...
	private  BGP4Update createMsgUpdateMDPCENLRI(String domainID, PCEInfo IP){
		try{
			...
			log.debug("Creating PCE Update related to Domain "+domainID);
...
//		}
	...
	public Inet4Address IntToInet4Address (Integer value) throws UnknownHostException {
