...
public class BGP4Parameters {
	...
	/**
	 * True: The values of BGPIdentifier and myAutonomousystem are set from the env file
	 * False: The values of BGPIdentifier and myAutonomousystem are set from the XML files
	 */
	 ...
	 * Constructor 
	 */
	BGP4Parameters(String confFile, ReaderParamsEnv envfile) throws IOException {
		peersToConnect =new LinkedList<BGP4LSPeerInfo>();
		...
		if (confFile!=null)
		{
			...
			if(envfile.getIdentifier()!=null) {

				BGPIdentifier = envfile.getIdentifier();
				myAutonomousSystem = Integer.parseInt(envfile.getDomainID());
				Indicator = true;
			}
				...
				public void endElement(String uri, String localName,
					...
					if(qName.equalsIgnoreCase("BGP4Port")) {
						...
						version = Integer.parseInt(tempVal.trim());
							...
							log.info("myAutonomousSystem:  " +myAutonomousSystem +" Indicator: " +Indicator);
					}
					...
					{//El BGP Identifier es la local BGP Address.
							...
							if(!Indicator)
							BGPIdentifier = tempVal.trim();
					log.info("BGP Identifier:  " +BGPIdentifier +" Indicator: " +Indicator);
					}
