...
public class TopologyReaderXML extends TopologyReader
{
	...
	public TopologyReaderXML(TopologiesDataBase topologies, TopologyModuleParams params, Lock lock, ReaderParamsEnv envfile)
	{
		super(topologies,params,lock, envfile);
	...
	public void readTopology() 
	{
		...
		if (params.getModexml()!=null){
			if (params.getModexml().equals("IP")){
				...
				params.setIdentifier(envfile.getIdentifier());
				ted.initializeFromFile(params.getNetworkDescriptionFile(), params.getIdentifier(), true, envfile);
			}
